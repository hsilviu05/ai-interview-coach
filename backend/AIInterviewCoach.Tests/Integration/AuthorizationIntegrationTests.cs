...
namespace AIInterviewCoach.Tests.Integration
{
    ...
    //       Admin*                     = Admin only
    //
    // Token-lookup endpoints (GET + POST /api/interviews/token/{...}) require authentication.
    // The frontend redirects to login before the candidate reaches the token-entry step.
